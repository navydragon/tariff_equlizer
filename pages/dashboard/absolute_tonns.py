...
def layout():
    return html.Div([
        dbc.Button(
            "+", id="fade-button_tonns", className="mb-3", n_clicks=0
        ),
        dbc.Fade(
            dbc.Card(
                dbc.CardBody(
...
def filters():
    return (
        html.Div([
            html.H2('Объем перевозок', className='my-section__title'),
            dbc.Row([
                dbc.Col([
                    html.Label('млн тонн'),
                    dcc.Dropdown(
                        id='group_parameter_tonns',
                ...
                dbc.Col(
                    html.Button(className='my-section__badge', id='btn-excel-export-tonns-ar', style={'margin-left': 'auto', 'margin-right':'10px'}, children='Экспорт'),
                width=2)
            ], className='mb-3'),
            dcc.Loading(
                id="loading",
                ...
                color="#e21a1a",
                fullscreen=False,
                children=[html.Div([], id='absolute_tonns_table_div')]
            ),
        ])
    )
...
def group_and_combine(df, loss_df, group1,group2, market_loss):
    ...
    for index, row in df_gr.iterrows():
        ...
        for year_index, year in enumerate([2024] + years):
            if market_loss:
                year_value = loss_row[f'cargo_after_loss_{year}'] / 1000000
            else:
                year_value = row[f'2024 Объем перевозок, т.'] / 1000000
...
def get_callbacks():
    @callback(
        Output("fade_tonns", "is_in"),
        Output("fade-button_tonns", "children"),
        [Input("fade-button_tonns", "n_clicks")],
        [State("fade_tonns", "is_in")],
    )
    ...
    @callback(
        Output("absolute_tonns_table_div", "children"),
        # Input('calculate-button', 'n_clicks'),
        Input("group_parameter_tonns", "value"),
        Input("group_parameter2_tonns", "value"),
    ...
    clientside_callback(
        ...
        Output("btn-excel-export-tonns-ar", "n_clicks"),
        Input("btn-excel-export-tonns-ar", "n_clicks"),
        prevent_initial_call=True
    )
