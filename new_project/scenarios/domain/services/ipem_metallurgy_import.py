"""Импорт металлургических model-маршрутов и правил эластичности из IPEM XLSX."""
...
@dataclass
class IpemMetallurgy2026BundleResult:
    seed: IpemElasticitySeedResult | None
    routes: object
    matching: ElasticityMatchingStats


def resolve_elasticity_rules_for_scenario(
    scenario: Scenario,
) -> list[ElasticityRule]:
...
def import_ipem_metallurgy_2026_bundle(
    scenario: Scenario,
    xlsx_path: Path,
    route_set: RouteSet,
    *,
...
) -> IpemMetallurgy2026BundleResult:
    seed_result: IpemElasticitySeedResult | None = None
    ...
    routes_result = import_ipem_metallurgy_2026_model_routes(
        xlsx_path,
        route_set,
        dry_run=dry_run,
        progress=progress,
    ...
    return IpemMetallurgy2026BundleResult(
        seed=seed_result,
