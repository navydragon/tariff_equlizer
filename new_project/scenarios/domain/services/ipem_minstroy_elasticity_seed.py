"""Seed ElasticitySet 2026 (minstroy) from IPEM 'Технический лист'."""
...
MINS_RULE_NAME = "IPEM: Минстрой"
MINS_CARGO_GROUP_CODE = 7
MINS_RULE_POSITION = 35


@dataclass(frozen=True)
class IpemMinstroyElasticitySeedResult:
...
def _default_workbook_path() -> Path:
    return _repo_root() / "data" / "ipem" / "Минстрой_эластика.xlsx"
...
@transaction.atomic
def seed_ipem_minstroy_elasticity_for_scenario(
    scenario: Scenario,
    *,
    author: User | None = None,
    attach: bool = True,
    xlsx_path: Path | None = None,
) -> IpemMinstroyElasticitySeedResult:
    """
    Создаёт/обновляет правило эластичности «IPEM: Минстрой» по листу
    «Технический лист» (одна кривая на cargo_group=7).
    """
    ...
    try:
        ...
        raise RuntimeError(
            "openpyxl is required to seed IPEM minstroy elasticity",
        ) from exc

    workbook_path = xlsx_path or _default_workbook_path()
    if not workbook_path.exists():
        return IpemMinstroyElasticitySeedResult(
            elasticity_set_id=0,
            rules_upserted=0,
            points_upserted=0,
    ...
    minstroy_group = _resolve_cargo_group_by_code(MINS_CARGO_GROUP_CODE)
    rule = ElasticityRule.objects.create(
        elasticity_set=elasticity_set,
        name=MINS_RULE_NAME,
        position=MINS_RULE_POSITION,
        cargo_group=minstroy_group,
    )
