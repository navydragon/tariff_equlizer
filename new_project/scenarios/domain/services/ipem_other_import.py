"""Импорт model-маршрутов прочих грузов и правил эластичности из IPEM XLSX."""
...
from core.management.ipem_economics import import_ipem_other_model_routes
from core.models import Route, RouteSet
from scenarios.domain.services.ipem_other_elasticity_seed import (
    IpemOtherElasticitySeedResult,
    seed_ipem_other_elasticity_for_scenario,
)
...
@dataclass
class IpemOtherBundleResult:
    seed: IpemOtherElasticitySeedResult | None
    routes: object
...
def import_ipem_other_bundle(
    ...
    *,
...
) -> IpemOtherBundleResult:
    seed_result: IpemOtherElasticitySeedResult | None = None
    if attach_elasticity:
        seed_result = seed_ipem_other_elasticity_for_scenario(
            scenario,
    ...
    routes_result = import_ipem_other_model_routes(
        xlsx_path,
        route_set,
        dry_run=dry_run,
        progress=progress,
