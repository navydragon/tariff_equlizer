"""Импорт model-маршрутов Минстроя и правил эластичности из IPEM XLSX."""
...
from scenarios.domain.services.ipem_minstroy_elasticity_seed import (
    IpemMinstroyElasticitySeedResult,
    seed_ipem_minstroy_elasticity_for_scenario,
)
...
@dataclass
class IpemMinstroyBundleResult:
    seed: IpemMinstroyElasticitySeedResult | None
    routes: object
...
def import_ipem_minstroy_bundle(
    scenario: Scenario,
    xlsx_path: Path,
    route_set: RouteSet,
    *,
...
) -> IpemMinstroyBundleResult:
    seed_result: IpemMinstroyElasticitySeedResult | None = None
    if attach_elasticity:
        seed_result = seed_ipem_minstroy_elasticity_for_scenario(
            scenario,
    ...
    return IpemMinstroyBundleResult(
        seed=seed_result,
