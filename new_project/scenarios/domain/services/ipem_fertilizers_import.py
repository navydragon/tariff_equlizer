"""Импорт model-маршрутов удобрений и правил эластичности из IPEM XLSX."""
...
from core.management.ipem_economics import import_ipem_fertilizers_model_routes
from core.models import Route, RouteSet
from scenarios.domain.services.ipem_fertilizers_elasticity_seed import (
    IpemFertilizersElasticitySeedResult,
    seed_ipem_fertilizers_elasticity_for_scenario,
)
...
@dataclass
class IpemFertilizersBundleResult:
    seed: IpemFertilizersElasticitySeedResult | None
    routes: object
...
def import_ipem_fertilizers_bundle(
    scenario: Scenario,
    xlsx_path: Path,
    route_set: RouteSet,
    *,
...
) -> IpemFertilizersBundleResult:
    seed_result: IpemFertilizersElasticitySeedResult | None = None
    if attach_elasticity:
        seed_result = seed_ipem_fertilizers_elasticity_for_scenario(
            scenario,
    ...
    routes_result = import_ipem_fertilizers_model_routes(
        xlsx_path,
        route_set,
        dry_run=dry_run,
        progress=progress,
    ...
    return IpemFertilizersBundleResult(
        seed=seed_result,
