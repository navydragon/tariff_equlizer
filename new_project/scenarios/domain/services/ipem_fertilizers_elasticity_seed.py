"""Seed ElasticitySet 2026 (fertilizers) from IPEM 'Технический лист'."""
...
FERTILIZERS_RULE_NAME = "IPEM: Удобрения"
FERTILIZERS_CARGO_GROUP_CODE = 8
FERTILIZERS_RULE_POSITION = 30


@dataclass(frozen=True)
class IpemFertilizersElasticitySeedResult:
...
def _default_workbook_path() -> Path:
    return _repo_root() / "data" / "ipem" / "Удобрения_эластика.xlsx"
...
@transaction.atomic
def seed_ipem_fertilizers_elasticity_for_scenario(
    scenario: Scenario,
    *,
    author: User | None = None,
    attach: bool = True,
    xlsx_path: Path | None = None,
) -> IpemFertilizersElasticitySeedResult:
    """
    Создаёт/обновляет правило эластичности «IPEM: Удобрения» по листу
    «Технический лист» (одна кривая на cargo_group=8).

    Угольные и металлургические правила не затрагиваются.
    """
    ...
    try:
        ...
        raise RuntimeError(
            "openpyxl is required to seed IPEM fertilizers elasticity",
        ) from exc

    workbook_path = xlsx_path or _default_workbook_path()
    if not workbook_path.exists():
        return IpemFertilizersElasticitySeedResult(
            elasticity_set_id=0,
            rules_upserted=0,
            points_upserted=0,
    ...
    ElasticityRule.objects.filter(
        ...
        name=FERTILIZERS_RULE_NAME,
    ).delete()

    fertilizers_group = _resolve_cargo_group_by_code(
        FERTILIZERS_CARGO_GROUP_CODE,
    )
    rule = ElasticityRule.objects.create(
        elasticity_set=elasticity_set,
        name=FERTILIZERS_RULE_NAME,
        position=FERTILIZERS_RULE_POSITION,
        cargo_group=fertilizers_group,
    )
