...
Создаёт или обновляет сценарий **«Базовый сценарий»** (2026–2035): технический `RouteSet`, BTD, наборы **«Прогноз ЦБ»** (инфляция и USD/RUB по данным Банка России).
...
Базовые KPI (погрузка, грузооборот, доходы) берутся из колонок **2026** (`2026 Погрузка,т`, `2026 Грузоб,ткм`, `2026 Доходы,руб`).
...
После смены базового года импорта перезагрузите маршруты: `python manage.py import_rzd_routes --clear`.
...
| `--skip-elasticity` | — | Только model-маршруты и линковка; **не** создавать и не перезаписывать правила эластичности |
| `--dry-run` | — | Только проверка резолва |
| `--only` | — | Запустить только указанные имена команд |

```bash
# Первичный импорт (seed правил из XLSX)
python manage.py import_ipem_all_routes --scenario-id 1 --route-set-code RZD_2026

# Повторный импорт после ручных правок кривых в UI — правила не трогать
...
Импорт **model-маршрутов** прочих грузов из `Прочие_эластика.xlsx` (лист `Прочие`, cargo_group=10).  
С `--scenario-id` — правило эластичности «IPEM: Прочие» (лист `Технический лист`).

| Параметр | По умолчанию | Описание |
|----------|--------------|----------|
| `--file` | `../data/ipem/Прочие_эластика.xlsx` | XLSX IPEM |
| `--route-set-code` | `RZD_2026` | Набор маршрутов |
| `--scenario-id` | — | Сценарий: seed эластичности + привязка набора |
| `--skip-elasticity` | — | Только маршруты |
| `--dry-run` | — | Только проверка резолва |
