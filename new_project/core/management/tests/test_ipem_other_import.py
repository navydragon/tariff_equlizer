...
def _other_xlsx_path() -> Path:
    return (
        Path(__file__).resolve().parents[4]
        / "data"
        / "ipem"
        / "Прочие_эластика.xlsx"
    )


class IpemOtherImportTests(TestCase):
    def setUp(self) -> None:
        self.route_set = RouteSet.objects.create(
            code="RZD_2026_OTHER",
            name="RZD other import",
        )
        ...
        self.wagon_kind = WagonKind.objects.create(code="WK_RF", name="Рефрижераторы")
        self.shipment_type = ShipmentType.objects.create(
            code="ST_C",
            name="контейнерная",
        )
        self.message_type = MessageType.objects.create(code="MT_INT", name="Внутр. перевозки")
    ...
    def test_clear_preserves_non_other_model_routes(self) -> None:
        forest_group = CargoGroup.objects.create(name="Лесные грузы", code=6, position=6)
        forest_cargo = Cargo.objects.create(
            code="09111",
            name="ПИЛМАТ ПР",
        ...
        clear_ipem_model_routes_for_cargo_groups(self.route_set, (10,))

        self.assertFalse(Route.objects.filter(pk=other_model.pk).exists())
        self.assertTrue(Route.objects.filter(pk=forest_model.pk).exists())

    def test_import_creates_model_route_with_other_prefix(self) -> None:
        resolved = IpemCoal2026ResolvedRow(
            ipem_row=1,
            route_code="IPEM-OTHER-2026-001",
            origin=self.origin,
