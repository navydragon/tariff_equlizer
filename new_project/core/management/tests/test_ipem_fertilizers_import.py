...
def _fertilizers_xlsx_path() -> Path:
    return (
        Path(__file__).resolve().parents[4]
        / "data"
        / "ipem"
        / "Удобрения_эластика.xlsx"
    )


class IpemFertilizersImportTests(TestCase):
    def setUp(self) -> None:
        self.route_set = RouteSet.objects.create(
            code="RZD_2026_FERT",
            name="RZD fertilizers import",
        )
        cargo_group = CargoGroup.objects.create(name="Удобрения", code=8, position=8)
        ...
        self.wagon_kind = WagonKind.objects.create(code="WK_HOP", name="Хопперы")
        ...
        Route.objects.create(
            ...
            route_code="OP-FERT-1",
            freight_charge_rub=Decimal("1000.00"),
        )

    def test_load_fertilizers_xlsx_reads_route_sheet(self) -> None:
        xlsx_path = _fertilizers_xlsx_path()
        if not xlsx_path.exists():
            self.skipTest(f"Файл IPEM не найден: {xlsx_path}")

        rows = load_ipem_fertilizers_xlsx(xlsx_path)
        self.assertGreaterEqual(len(rows), 40)
        self.assertIn("Код груза", rows[0])
        self.assertIn("Станц отпр РФ", rows[0])
        self.assertIn("Вид перевозки", rows[0])

    def test_parse_economics_from_fertilizers_row(self) -> None:
        xlsx_path = _fertilizers_xlsx_path()
        if not xlsx_path.exists():
            self.skipTest(f"Файл IPEM не найден: {xlsx_path}")
    ...
    def test_sanitize_ipem_text_strips_excel_apostrophe(self) -> None:
        from core.management.ipem_economics import sanitize_ipem_text

        self.assertEqual(sanitize_ipem_text("'Полувагоны"), "Полувагоны")
        self.assertEqual(sanitize_ipem_text("'433046"), "433046")

    def test_resolve_wagon_kind_aliases_for_fertilizers(self) -> None:
        from core.management.ipem_economics import resolve_wagon_kind

        other, _ = WagonKind.objects.get_or_create(
            code="WK_OTHER",
            defaults={"name": "Прочие"},
        )
        wagon, issue = resolve_wagon_kind("Минераловозы", [other])
        self.assertIsNone(issue)
        self.assertEqual(wagon, other)

    def test_clear_preserves_non_fertilizer_model_routes(self) -> None:
        ...
        fert_model = Route.objects.create(
            route_set=self.route_set,
            is_model=True,
            route_code="MODEL-FERT",
            cargo=self.cargo,
            origin_station=self.origin,
            destination_station=self.destination,
            wagon_kind=self.wagon_kind,
        ...
        clear_ipem_model_routes_for_cargo_groups(self.route_set, (8,))

        self.assertFalse(Route.objects.filter(pk=fert_model.pk).exists())
        self.assertTrue(Route.objects.filter(pk=coal_model.pk).exists())

    def test_import_creates_model_route_with_fert_prefix(self) -> None:
        resolved = IpemCoal2026ResolvedRow(
            ipem_row=1,
            route_code="IPEM-FERT-2026-001",
            origin=self.origin,
