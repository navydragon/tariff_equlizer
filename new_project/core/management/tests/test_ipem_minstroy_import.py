...
def _minstroy_xlsx_path() -> Path:
    return (
        Path(__file__).resolve().parents[4]
        / "data"
        / "ipem"
        / "Минстрой_эластика.xlsx"
    )


class IpemMinstroyImportTests(TestCase):
    def setUp(self) -> None:
        self.route_set = RouteSet.objects.create(
            code="RZD_2026_MINS",
            name="RZD minstroy import",
        )
        cargo_group = CargoGroup.objects.create(
            name="Минерально-строит.",
            code=7,
            position=7,
        )
        ...
        Route.objects.create(
            ...
            route_code="OP-MINS-1",
            freight_charge_rub=Decimal("1000.00"),
        )

    def test_load_minstroy_xlsx_reads_route_sheet(self) -> None:
        xlsx_path = _minstroy_xlsx_path()
        if not xlsx_path.exists():
            self.skipTest(f"Файл IPEM не найден: {xlsx_path}")

        rows = load_ipem_minstroy_xlsx(xlsx_path)
        self.assertGreaterEqual(len(rows), 20)
        self.assertIn("Код груза", rows[0])
        self.assertIn("Станц отпр РФ", rows[0])
        self.assertIn("Вид перевозки", rows[0])

    def test_parse_economics_from_minstroy_row(self) -> None:
        xlsx_path = _minstroy_xlsx_path()
        if not xlsx_path.exists():
            self.skipTest(f"Файл IPEM не найден: {xlsx_path}")
    ...
    def test_resolve_wagon_kind_aliases_for_minstroy(self) -> None:
    ...
    def test_clear_preserves_non_minstroy_model_routes(self) -> None:
        ...
        mins_model = Route.objects.create(
            route_set=self.route_set,
            is_model=True,
            route_code="MODEL-MINS",
            cargo=self.cargo,
            origin_station=self.origin,
            destination_station=self.destination,
            wagon_kind=self.wagon_kind,
        ...
        clear_ipem_model_routes_for_cargo_groups(self.route_set, (7,))

        self.assertFalse(Route.objects.filter(pk=mins_model.pk).exists())
        self.assertTrue(Route.objects.filter(pk=coal_model.pk).exists())

    def test_import_creates_model_route_with_mins_prefix(self) -> None:
        resolved = IpemCoal2026ResolvedRow(
            ipem_row=1,
            route_code="IPEM-MINS-2026-001",
            origin=self.origin,
