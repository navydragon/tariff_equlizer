...
IPEM_METALLURGY_CARGO_GROUP_CODES: tuple[int, ...] = (2, 4, 5)
...
IPEM_OIL_ROUTE_SHEETS: tuple[str, ...] = (
    "Нефтяные",
)
IPEM_OIL_XLSX_NAME = "Нефть_эластика.xlsx"
IPEM_OIL_HEADER_ROW = 2
IPEM_OIL_CARGO_GROUP_CODE = 3

IPEM_MINSTROY_ROUTE_SHEETS: tuple[str, ...] = (
    "Минстрой",
)
IPEM_MINSTROY_XLSX_NAME = "Минстрой_эластика.xlsx"
...
IPEM_OTHER_ROUTE_SHEETS: tuple[str, ...] = (
    "Прочие",
)
IPEM_OTHER_XLSX_NAME = "Прочие_эластика.xlsx"
IPEM_OTHER_HEADER_ROW = 2
IPEM_OTHER_CARGO_GROUP_CODE = 10
...
def load_ipem_minstroy_xlsx(path: Path) -> list[dict[str, str]]:
    """
    Импорт Minstroy-таблицы с нормализацией названий колонок.

    В отличие от других файлов, у `Минстрой_эластика.xlsx` иногда встречаются
    небольшие отличия в заголовках, из-за чего `row.get(<key>)` в резолвере
    не находит нужные поля. Здесь приводим заголовки к ожидаемым ключам через
    similarity-based мэппинг.
    """
    import difflib
    ...
    rows: list[dict[str, str]] = []
    for sheet_name in IPEM_MINSTROY_ROUTE_SHEETS:
        df = pd.read_excel(
            path,
            sheet_name=sheet_name,
            header=IPEM_MINSTROY_HEADER_ROW,
        )

        # Приводим "видимые" названия колонок к строкам без крайних пробелов.
        df.columns = [str(col).strip() for col in df.columns]
        actual_columns = list(df.columns)

        expected_columns = {
            # resolve_ipem_coal_2026_row:
            "Станц отпр РФ",
            "Станц назн РФ",
            "Дор отпр",
            "Дор наз",
            ...
            *IPEM_COAL_2026_COLUMN_BY_ROUTE_FIELD.values(),
            # частные fallback-и/alias-колонки:
        ...
        expected_norm = {k: normalize_name(k) for k in expected_columns}

        for expected_key in expected_columns:
            if expected_key in actual_columns:
                continue
            expected_key_norm = expected_norm[expected_key]
            ...
            for actual_col in actual_columns:
                ...
                ratio = difflib.SequenceMatcher(
                    None,
                    expected_key_norm,
                    normalize_name(actual_col),
                ).ratio()
                if ratio > best_ratio:
                    best_ratio = ratio
                    best_actual = actual_col

            # Threshold эмпирический: для "почти одинаковых" заголовков.
            if best_actual is not None and best_ratio >= 0.72:
                used_actual.add(best_actual)
                rename_map[best_actual] = expected_key
        ...
        for _, series in df.iterrows():
            row = {
                str(col).strip(): _ipem_cell_str(series[col])
...
def load_ipem_other_xlsx(path: Path) -> list[dict[str, str]]:
    import pandas as pd

    rows: list[dict[str, str]] = []
    for sheet_name in IPEM_OTHER_ROUTE_SHEETS:
        df = pd.read_excel(
            path,
            sheet_name=sheet_name,
            header=IPEM_OTHER_HEADER_ROW,
        )
        for _, series in df.iterrows():
            row = {str(col).strip(): _ipem_cell_str(series[col]) for col in df.columns}
            if not any(row.values()):
...
def load_ipem_oil_xlsx(path: Path) -> list[dict[str, str]]:
    import pandas as pd

    rows: list[dict[str, str]] = []
    for sheet_name in IPEM_OIL_ROUTE_SHEETS:
        df = pd.read_excel(
            path,
            sheet_name=sheet_name,
            header=IPEM_OIL_HEADER_ROW,
        )
        for _, series in df.iterrows():
            row = {str(col).strip(): _ipem_cell_str(series[col]) for col in df.columns}
            if not any(row.values()):
...
def import_ipem_minstroy_model_routes(
    xlsx_path: Path,
    route_set: RouteSet,
    *,
    dry_run: bool = False,
    ...
    """
    Импорт model-маршрутов из `Минстрой_эластика.xlsx` (лист «Минстрой»).

    Использует тот же формат колонок, что и угольные/металлургические листы.
    """
    result = IpemCoal2026ImportResult()
    ipem_rows = load_ipem_minstroy_xlsx(xlsx_path)
    result.total_rows = len(ipem_rows)
    ...
    for ipem_row_idx, row in enumerate(ipem_rows, start=1):
        ...
        resolved.route_code = f"IPEM-MINS-2026-{ipem_row_idx:03d}"
...
def import_ipem_other_model_routes(
    xlsx_path: Path,
    route_set: RouteSet,
    *,
    dry_run: bool = False,
    ...
    """
    Импорт model-маршрутов из `Прочие_эластика.xlsx` (лист «Прочие»).

    Использует тот же формат колонок, что и лесные/нефтяные листы.
    """
    result = IpemCoal2026ImportResult()
    ipem_rows = load_ipem_other_xlsx(xlsx_path)
    result.total_rows = len(ipem_rows)
    ...
    for ipem_row_idx, row in enumerate(ipem_rows, start=1):
        ...
        resolved.route_code = f"IPEM-OTHER-2026-{ipem_row_idx:03d}"
...
def import_ipem_oil_model_routes(
    xlsx_path: Path,
    route_set: RouteSet,
    *,
    dry_run: bool = False,
    ...
    """
    Импорт model-маршрутов из Нефть_эластика.xlsx (лист «Нефтяные»).

    Использует тот же формат колонок, что и угольные/металлургические листы.
    """
    result = IpemCoal2026ImportResult()
    ipem_rows = load_ipem_oil_xlsx(xlsx_path)
    result.total_rows = len(ipem_rows)
    ...
    for ipem_row_idx, row in enumerate(ipem_rows, start=1):
        ...
        resolved.route_code = f"IPEM-OIL-2026-{ipem_row_idx:03d}"
