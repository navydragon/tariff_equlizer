...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser) -> None:
        ...
        parser.add_argument(
            ...
            action="append",
            default=[],
            help="Путь к XLSX IPEM с model-маршрутами. Можно указывать несколько раз.",
        )
        parser.add_argument(
            "--dry-run",
            dest="dry_run",
    ...
    def handle(self, *args, **options) -> None:
            ...
            if not seed_only:
                for raw_path in file_paths:
                    ...
                    if "уголь" in name:
                        ...
                        raise CommandError(
                            f"Не удалось определить тип файла по имени {xlsx_path.name!r}. "
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Эластичность: set_id={seed_result.elasticity_set_id}, "
                f"правил={seed_result.rules_upserted}, точек={seed_result.points_upserted}"
                f"{', привязан к сценарию' if seed_result.attached_to_scenario else ''}."
            )
        )

        if seed_only:
            self.stdout.write(self.style.SUCCESS("Готово: только seed эластичности."))
            return
        ...
        for kind, rr in routes_results:
            ...
            if not dry_run:
                ...
                self.stdout.write(
                    f"[{kind}] разметка эластичности: direct_model={rr.elasticity_direct_model}, "
                    f"holding_aggregate={rr.elasticity_holding_aggregate}, "
                    f"cargo_group_aggregate={rr.elasticity_cargo_group_aggregate}, "
                    f"skip={rr.elasticity_skipped}"
                )
