...
from scenarios.domain.services.ipem_fertilizers_import import (
    import_ipem_fertilizers_bundle,
)
...
class Command(BaseCommand):
    help = (
        "Импорт model-маршрутов удобрений из Удобрения_эластика.xlsx в "
        "RouteSet и связка operational-маршрутов РЖД через model_route_id. "
        "С --scenario-id также загружает правила эластичности (лист "
        "«Технический лист», cargo_group=8) и выводит статистику матчинга."
    )
    ...
    def add_arguments(self, parser) -> None:
        parser.add_argument(
            "--file",
            dest="file_path",
            default="../data/ipem/Удобрения_эластика.xlsx",
            help="Путь к XLSX IPEM (удобрения)",
        )
        parser.add_argument(
            "--route-set-code",
            dest="route_set_code",
        ...
        *,
        ...
        with transaction.atomic():
            if use_bundle:
                ...
                result = import_ipem_fertilizers_bundle(
                    scenario,
                    xlsx_path,
                    route_set,
                    dry_run=dry_run,
