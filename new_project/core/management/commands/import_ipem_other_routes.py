...
from scenarios.domain.services.ipem_other_import import (
    import_ipem_other_bundle,
)
...
class Command(BaseCommand):
    help = (
        "Импорт model-маршрутов прочих грузов из Прочие_эластика.xlsx в "
        "RouteSet и связка operational-маршрутов РЖД через model_route_id. "
        "С --scenario-id также загружает правила эластичности (лист "
        "«Технический лист», cargo_group=10) и выводит статистику матчинга."
    )
    ...
    def add_arguments(self, parser) -> None:
        parser.add_argument(
            "--file",
            dest="file_path",
            default="../data/ipem/Прочие_эластика.xlsx",
            help="Путь к XLSX IPEM (прочие грузы)",
        )
        parser.add_argument(
            "--route-set-code",
            dest="route_set_code",
        ...
        *,
        ...
        with transaction.atomic():
            if use_bundle:
                ...
                result = import_ipem_other_bundle(
                    scenario,
                    xlsx_path,
                    route_set,
                    dry_run=dry_run,
