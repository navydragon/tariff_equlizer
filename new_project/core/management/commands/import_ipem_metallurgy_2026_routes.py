...
from scenarios.domain.services.ipem_metallurgy_import import (
    import_ipem_metallurgy_2026_bundle,
)
...
class Command(BaseCommand):
    help = (
        "Импорт model-маршрутов металлургии из Металлургия_эластика.xlsx в "
        "RouteSet и связка operational-маршрутов РЖД через model_route_id. "
        "С --scenario-id также загружает правила эластичности (лист "
        "«Технический лист», только руда/металлы) и выводит статистику матчинга."
    )
    ...
    def add_arguments(self, parser) -> None:
        parser.add_argument(
            ...
            help="Путь к XLSX IPEM (металлургия)",
        )
        parser.add_argument(
            "--route-set-code",
            dest="route_set_code",
        ...
        *,
        ...
        with transaction.atomic():
            if use_bundle:
                ...
                result = import_ipem_metallurgy_2026_bundle(
                    scenario,
                    xlsx_path,
                    route_set,
                    dry_run=dry_run,
