  ...
  class RouteAnalysisAssistantController extends Stimulus.Controller {
    ...
    _onContextUpdated(event) {
      ...
      this.pageContext = {
        ...
        routeId:
          detail.routeId ??
          (route && route.id != null ? route.id : null) ??
          null,
    ...
    async _send({ message = null, preset = null, userLabel }) {
      ...
      try {
        ...
        this._dispatchUiAction(data.ui_action);
      } catch (err) {
    ...
    _dispatchUiAction(uiAction) {
      if (!uiAction || typeof uiAction !== "object") return;
      const overrides = uiAction.apply_equalizer_overrides;
      if (!overrides || typeof overrides !== "object") return;
      document.dispatchEvent(
        new CustomEvent("route-analysis:apply-equalizer-overrides", {
          detail: {
            overrides,
            focusType: uiAction.focus_equalizer_type || null,
          },
        }),
      );
    }
    ...
    _appendMessage(role, text) {
      const messages = this.element.querySelector(
        '[data-route-analysis-assistant-target="messages"]',
      );
      if (!messages) return;
      const emptyHint = messages.querySelector(".text-muted.small");
    ...
    _showErrors(errors) {
      const errorsEl = this.element.querySelector(
        '[data-route-analysis-assistant-target="errors"]',
      );
      if (!errorsEl) return;
